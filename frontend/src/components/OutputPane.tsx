...
function LetterBody({ text, citations, onOpen }: { text: string; citations: Citation[]; onOpen: (c: Citation) => void }) {
  return (
    <>
      {text.split("\n\n").map((para, pi) => (
        <p key={pi} className="mb-3 text-[13.5px] leading-relaxed text-slate-700">
          {para.split(MARKER_SPLIT).map((part, i) => {
            if (IS_MARKER.test(part)) {
              const section = part.slice(1, -1).split("§")[1]?.trim() ?? "";
              const cite =
                citations.find((c) => section && c.section_heading.includes(section)) ??
                citations.find((c) => section.includes(c.section_heading));
              return <CitationChip key={i} label={part} onClick={() => cite && onOpen(cite)} />;
            }
        ...
        {hasLetter && (
          <div>
            <div className="mb-4 flex items-center gap-2">
              ...
              <span className="text-xs text-muted">Filing-ready · pending human sign-off</span>
            </div>
        ...
        {gap && (
          <div>
            <div className="mb-4 flex items-center gap-2">
              ...
              <span className="text-xs text-muted">Coverage {gap.coverage}</span>
            </div>

            <div className="card p-5">
              <h3 className="text-sm font-semibold text-ink">Gap report</h3>
              <p className="mt-1 text-[13px] text-slate-600">
                This case is not yet appealable. Obtain the items below before filing — the deadline leaves{" "}
                <span className="font-semibold text-ink">{gap.days_remaining} days</span> (by {gap.deadline}).
              </p>

              <div className="mt-4 space-y-2">
                {gap.unmet?.map((u: any) => (
                  <div key={u.criterion_id} className="rounded-lg border border-red-200 bg-red-50 p-3">
                    <div className="flex items-center gap-2 text-xs font-semibold text-red-700">
                      <IconX className="h-3.5 w-3.5" /> §{u.criterion_id} not met
                    </div>
                    <p className="mt-1 text-[13px] text-slate-700">{u.rationale}</p>
                  </div>
                ))}
                {gap.unverifiable?.map((u: any) => (
                  <div key={u.criterion_id} className="rounded-lg border border-amber-200 bg-amber-50 p-3">
                    <div className="flex items-center gap-2 text-xs font-semibold text-amber-700">
                      <IconQuestion className="h-3.5 w-3.5" /> §{u.criterion_id} cannot verify
                    </div>
                    <p className="mt-1 text-[13px] text-slate-700">{u.documentation_request}</p>
                  </div>
                ))}
              </div>
